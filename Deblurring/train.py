...
import utils
from data_RGB import get_training_data2, get_validation_data2
...
# train_dir_LQ = opt.TRAINING.TRAIN_DIR_LQ
# train_dir_GT = opt.TRAINING.TRAIN_DIR_GT
#val_dir_LQ   = opt.TRAINING.VAL_DIR_LQ
#val_dir_GT   = opt.TRAINING.VAL_DIR_GT
...
# train_dataset = get_training_data(train_dir_LQ, train_dir_GT)
train_dataset = get_training_data2(train_dir)
...
val_dataset = get_validation_data2(val_dir)
...
for epoch in range(start_epoch, opt.OPTIM.NUM_EPOCHS + 1):
    ...
    #print("get_lr = ", scheduler.get_lr())
    #print("get_last_lr = ", scheduler.get_last_lr())
    ...
    for i, data in enumerate(tqdm(train_loader), 0):
        ...
        target_hr = data[1].cuda()
        # print("before assert input_.shape: ", input_.shape)
        assert input_.shape == torch.Size([opt.OPTIM.BATCH_SIZE,3,64,64])
        assert target_hr.shape == torch.Size([opt.OPTIM.BATCH_SIZE,3,256,256])
        # print("after to_tensor", input_)
        ...
        restored_sr = model_restoration(input_)
        # print("restored_sr = ", restored_sr.size())
        ...
        loss = l1_loss(restored_sr, target_hr)
